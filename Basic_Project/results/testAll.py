		...
		for order_O in ordersA:
			...
			for thresh in thresholdA:
				...
				if float(acc_f) > float(bests['acc'].split( )[1]):
					toAdd = str(smoothing_algo) + "_" + str(order) + "_" + str(threshold +  " " + float(acc_f)
					bests['acc'] = toAdd
				if float(pre_f) > float(bests['pre'].split( )[1]):
					bests['pre'] =  str(smoothing_algo) + "_" + str(order) + "_" + str(threshold) + " " + float(pre_f)
				if float(rec_f) > float(bests['rec'].split( )[1]):
					bests['rec'] =  str(smoothing_algo) + "_" + str(order) + "_" + str(threshold) + " " + float(rec_f)
				if float(fb1_f) > float(bests['fb1'].split( )[1]):
					bests['fb1'] =  str(smoothing_algo) + "_" + str(order) + "_" + str(threshold) + " " + float(fb1_f)
